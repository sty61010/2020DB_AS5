...
public class RecordPage implements Record {
	...
	private Constant getVal(int offset, Type type) {
		RecordId ri = new RecordId(blk, currentSlot);
		if (!isTempTable()) {
			tx.concurrencyMgr().readRecord(ri);
		}
		Constant val = tx.ReadModified(ri, offset);
...
//		currentBuff.setVal(offset, val, tx.getTransactionNumber(), lsn);
		tx.addWriteInfo(lsn, offset, val, blk, currentSlot);	
	}
